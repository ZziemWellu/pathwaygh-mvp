...
class RealGhanaDataLoader:
    ...
    def get_career_demand(self, career: str) -> Dict:
        ...
        career_demand_data = self.job_market.get('career_demand_data', {})
        
        # Exact match
        if career in career_demand_data:
            return career_demand_data[career]
        
        # Case-insensitive match
        for key in career_demand_data:
            if key.lower() == career.lower():
                return career_demand_data[key]
        
        # Return default empty dict with some defaults
        return {
            'demand_level': 'Good',
            'growth_rate': '5% annually',
            'starting_salary': 2000,
            'experienced_salary': 8000,
            'top_employers': ['Varies by sector']
        }
